...
;(function($) {
   ...
   var dd = function(element, options)
   {
		...
		ddList = (document.getElementById(elementid).size>1 || document.getElementById(elementid).multiple==true) ? true : false;
	...
	var applyEventsOnA = function() {
		var childid = getPostID("postChildID");
		$("#"+childid+ " a.enabled").unbind("click");
			$("#"+childid+ " a.enabled").bind("click", function(event) {
														//alert(event.target);
														 event.preventDefault();
														 manageSelection(event.target);
														 if(!ddList) {
	...
	var createDropDown = function () {
		...
		var childid = getPostID("postChildID");
		var arrowid = getPostID("postArrowID");
		var iWidth = $("#"+elementid).attr('width'); //update to get width of selection box
        //alert(elementid + "w=" + iWidth);
	...
	var manageSelection = function (obj) {
		...
		if(document.getElementById(elementid).selectedIndex>=0) {
			oldSelectedValue = document.getElementById(elementid).options[document.getElementById(elementid).selectedIndex].text;
		} else {
			oldSelectedValue = "";
		};
	...
	var checkMethodAndApply = function () {
		...
		if(has_handler('change')==true) {
			...
			if(oldSelectedValue!=currentSelectedValue){ //oldSelectedValue is assigned on manageSelection method
	...
	var shouldOpenOpposite = function() {
		...
		if( wH< Math.floor(cH+mH+pos.top) ) {
			var tp = pos.top-cH;
			if((pos.top-cH)<0) {
				tp = 10;
			};
			css = {zIndex:options.zIndex, top:tp+"px", display:"block"};
			ani = "show";
			opp = true;
	...
	this.get = function(prop, forceRefresh) {
		if(prop==undefined && forceRefresh==undefined) {
			//return all properties if no property is passed
		 	return $this.ddProp;
		};
		if(prop!=undefined && forceRefresh==undefined) {
